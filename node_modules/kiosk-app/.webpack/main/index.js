...
const createWindow = () => {
    mainWindow = new electron_1.BrowserWindow({
        ...
        width: 800,
        webPreferences: {
            preload: 'C:\\Users\\A\\Desktop\\daesung_kiosk\\kiosk-app\\.webpack\\renderer\\main_window\\preload.js',
        },
    });
...
function connectCardReader() {
    ...
    // ETX(0x03)를 구분자로 사용 (카드리더기 STX/ETX 프로토콜)
    const parser = port.pipe(new serialport_1.DelimiterParser({ delimiter: Buffer.from([0x03]) }));
    parser.on('data', (data) => {
        // STX(0x02) 제거 후 카드 번호 추출
        const raw = data.toString('utf-8');
        const value = raw.replace(/\x02/g, '').trim();
        if (value.length === 0)
            return;
